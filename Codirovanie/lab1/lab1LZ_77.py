...
значения в выходной файл"""
    try:
        with open(input_file, "rb") as f:
            input_codes = str(pickle.load(f))
            input_encoded = str(pickle.load(f))
        ...
        raise
    print("Wait, please...")
    compressed_input1 = compress(input_codes)
    compressed_input2 = compress(input_encoded)

    with open(output_file, "wb") as f:
        pickle.dump(compressed_input1, f)
        pickle.dump(compressed_input2, f)
...
значения в выходной файл"""
    try:
        with open(input_file, "rb") as f:
            input_codes = pickle.load(f)
            input_encoded = pickle.load(f)
    except FileNotFoundError:
        print(f"Could not find input file at: {input_file}")
        raise
    except Exception:
    ...
    print('Wait, please...')
    compressed_input1 = decompress(input_codes)
    compressed_input2 = decompress(input_encoded)
...
req = input("Команда: ")
if req == 'encode':
    compress_file("C:\EmilyVolkova\Codirovanie\lab1\compressed_by_Huff.huf", "C:\EmilyVolkova\Codirovanie\lab1\compressed_by_LZ77.huf")
elif req == 'decode':
    decompress_file("C:\EmilyVolkova\Codirovanie\lab1\compressed_by_LZ77.huf", "C:\EmilyVolkova\Codirovanie\lab1\decompressed_by_LZ77.huf")
else:
